...
class OpentelemetryLogParser(BaseLogParser):
    ...
    def __init__(self, config: Config):
        ...
        if otel_config["text_message_parser"]["enable"]:
            ...
            if otel_config["text_message_parser"].has_key("regex"):
                self.regex_log_parser.set_regex(
                    otel_config["text_message_parser"]["regex"]
                )
    ...
    def parse(self, otel_record: OpentelemetryAccessLogRecord) -> dict[str, Any]:
        ...
        if self.regex_log_parser is not None:
            dict_recursive_update(
                log_dict, self.regex_log_parser.parse(otel_record.message)
            )
    ...
    def read_otel_mdc(
        ...
        app_path = (
            "/"
            if attributes.get("app_path_is_root")
            else self.get_app_path(request_path)
        )
        ...
        log_dict = {
            ...
            "request_query_string": attributes.get(
            ...
            "request_details": {
                ...
                if k.startswith("http.request.parameter.")
            },
            "response_time": int_or_none(
                attributes.get("http.response.duration_ms", "")
            ),
            "response_size": int_or_none(
                attributes.get("http.response.body.size_bytes", "")
            ),
