from georchestra_analytics_cli.access_logs.app_processors.mapproxy import MapproxyLogProcessor


def test_is_relevant():
    lp = MapproxyLogProcessor(app_path="")
    assert lp.is_relevant("/administratif/service", "SERVICE=WFS&VERSION=1.1.0&REQUEST=GetCapabilities") == True
    assert lp.is_relevant("/ortho/ows", "transparent=True&layers=ortho_2022&format=image/png&bbox=790678.4431818479,6543020.723557551,790893.4171401801,6543235.6975158835&width=1300&height=1300&srs=EPSG:2154&request=GetMap&version=1.1.1&service=WMS&styles=") == True
...
def test_collect_information():
    lp = MapproxyLogProcessor(config={
        "infer_is_tiled": True
    })
    req_path = "/ortho/service"
