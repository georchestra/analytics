...
CREATE
MATERIALIZED VIEW analytics.geoserver_summary_hourly
...
SELECT time_bucket(INTERVAL '1 h', ts, 'Europe/Paris') AS bucket_hourly,
    ...
    count (id) AS nb_req, AVG (response_time) AS avg_time,
...
GROUP BY bucket_hourly, user_name, org_name, request_method, status_code, request_details ->> 'layers',
    ...
    request_details ->> 'is_download', request_details ->> 'download_format';
...
SELECT add_retention_policy('analytics.geoserver_summary_hourly', drop_after = > INTERVAL '3 weeks',
...
SELECT add_retention_policy('analytics.geoserver_summary_daily', drop_after = > INTERVAL '2 years',
